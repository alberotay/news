...
function fillDesktopGrid(res) {
    ...
    res.forEach((y) => {
     ...
     $('#' + source + '_newsContent_' + j).append('<div class="news-date-icon"><span class="news-date">' + new Date(feed.pubDate).toLocaleString() + '</span><i class="bi bi-box-arrow-down news-icon" id="' + source + '_verMas_' + j + '"></i></div>');
    
     $('#' + source + '_newsContent_' + j).append('<div id="' + source + '_newsDescription_' + j + '" class="news-desciption" />');
     $('#' + source + '_newsDescription_' + j).append('<p>' + feed.description + '</p>');
